...
public class AssistantServiceImpl implements AssistantService {
    ...
    public AssistantServiceImpl(ChatRepository chatRepository, ChatMessageImageRepository chatMessageImageRepository, GeneralService generalService, StudentRepository studentRepository) {
    ...
    @Override
    public ControllerResponse sendMsgToStudent(Employee employee, Integer task, String text, MultipartFile image) {
    ...
    public String getMsg() {
        final Optional<Chat> byId = chatRepository.findById(4);

        final List<Chat> byStudentAndLessonCourseOrderByCreatedDateDesc = chatRepository
                .findByStudentAndLessonCourseOrderByCreatedAtDesc(byId.get().getStudent(), byId.get().getLesson().getCourse());
